"""
Tests for eclipse southern limit calculation in libephemeris.

Tests the calculation of geographic coordinates (latitude, longitude) for points
along the southern limit of the umbral/antumbral shadow path during solar eclipses.
...
class TestEclipseSouthernLimitBasicFunctionality:
    """Test basic functionality of calc_eclipse_southern_limit."""

    def test_function_exists(self):
        """Test that calc_eclipse_southern_limit function exists and is callable."""
        from libephemeris.eclipse import calc_eclipse_southern_limit as func

        assert callable(func)

    def test_function_exported_from_main_module(self):
        """Test that function is exported from main libephemeris module."""
        from libephemeris import calc_eclipse_southern_limit as func
    ...
    def test_returns_three_tuples(self):
        ...
        times_ecl, ecl_type = sol_eclipse_when_glob(jd_start, eclipse_type=SE_ECL_TOTAL)
        jd_c1 = times_ecl[1]  # First contact
        jd_c4 = times_ecl[4]  # Fourth contact

        result = calc_eclipse_southern_limit(jd_c1, jd_c4, step_minutes=30.0)
    ...
    def test_returns_same_length_tuples(self):
        ...
        jd_start = julday(2024, 1, 1, 0.0)
        times_ecl, _ = sol_eclipse_when_glob(jd_start, eclipse_type=SE_ECL_TOTAL)
        jd_c1 = times_ecl[1]
        jd_c4 = times_ecl[4]

        times, lats, lons = calc_eclipse_southern_limit(jd_c1, jd_c4, step_minutes=30.0)
...
class TestEclipseSouthernLimitTotalEclipse:
    """Test southern limit calculation for total eclipses."""

    def test_april_2024_total_eclipse_southern_limit(self):
        """Test April 8, 2024 total solar eclipse southern limit.

        NASA Reference: The April 8, 2024 total eclipse path crosses
        Mexico, USA, and Canada. The southern limit should be south of
        ...
        # Calculate southern limit with 10-minute steps
        times, lats, lons = calc_eclipse_southern_limit(jd_c1, jd_c4, step_minutes=10.0)

        # Should return points along the southern limit
        assert len(times) > 0, "Should have southern limit points"
...
class TestEclipseSouthernLimitAnnularEclipse:
    """Test southern limit calculation for annular eclipses."""

    def test_annular_eclipse_southern_limit(self):
        """Test annular eclipse southern limit calculation."""
        jd_start = julday(2023, 1, 1, 0.0)
        times_ecl, ecl_type = sol_eclipse_when_glob(
...
class TestEclipseSouthernLimitVsCentralLine:
    """Test that southern limit is correctly positioned relative to central line."""

    def test_southern_limit_generally_south_of_central_line(self):
        """Test that southern limit latitude is generally <= central line latitude.
        ...
        # Calculate central line and southern limit near maximum
        ...
        times_south, lats_south, lons_south = calc_eclipse_southern_limit(
            jd_c1, jd_c4, step_minutes=5.0
        )

        # Both should have points
        assert len(times_central) > 0, "Should have central line points"
        assert len(times_south) > 0, "Should have southern limit points"

        # For matching times, the southern limit should generally be south
        # (or very close to) the central line
        south_count = 0
        total_count = 0

        for i, t_south in enumerate(times_south):
            # Find closest central line point by time
            if times_central:
                closest_central_idx = min(
                    range(len(times_central)),
                    key=lambda j: abs(times_central[j] - t_south),
                )
                # Only compare if times are very close (within 1 minute)
                if abs(times_central[closest_central_idx] - t_south) < 1.0 / (24 * 60):
                    total_count += 1
                    # The southern limit should be at same or lower latitude
                    # Allow small tolerance for numerical precision
                    if lats_south[i] <= lats_central[closest_central_idx] + 0.5:
                        south_count += 1

        # At least 50% of points should satisfy the southern relationship
        # (accounting for edge effects and projection issues)
        if total_count > 0:
            ratio = south_count / total_count
            assert ratio >= 0.5, (
                f"Expected southern limit to be south of central line for most points, "
    ...
    def test_southern_limit_different_from_central_line(self):
        ...
        if len(lats_central) > 0 and len(lats_south) > 0:
            # The latitudes should be different (offset by umbral radius)
            lat_diff = abs(lats_south[0] - lats_central[0])
            ...
            assert lat_diff >= 0.0, "Southern limit should differ from central line"


class TestEclipseSouthernLimitStepSize:
    """Test southern limit calculation with different step sizes."""
    ...
    def test_smaller_step_gives_more_points(self):
        ...
        times_1min, _, _ = calc_eclipse_southern_limit(jd_c1, jd_c4, step_minutes=1.0)
        times_10min, _, _ = calc_eclipse_southern_limit(jd_c1, jd_c4, step_minutes=10.0)
    ...
    def test_default_step_is_one_minute(self):
        ...
        # Use default step
        times_default, _, _ = calc_eclipse_southern_limit(jd_c1, jd_c4)
...
class TestEclipseSouthernLimitEdgeCases:
    """Test edge cases for southern limit calculation."""
...
class TestEclipseSouthernLimitMultipleEclipses:
    """Test southern limit calculation across multiple eclipses."""

    def test_multiple_eclipses_have_valid_southern_limits(self):
        """Test that multiple eclipses all produce valid southern limits."""
...
class TestEclipseSouthernLimitPhysicalReasonableness:
    """Test physical reasonableness of southern limit calculations."""

    def test_southern_limit_offset_reasonable(self):
        """Test that the offset between central line and southern limit is reasonable.
...
class TestEclipseSouthernLimitVsNorthernLimit:
    """Test that southern limit is correctly positioned relative to northern limit."""

    def test_southern_limit_south_of_northern_limit(self):
        """Test that southern limit is always south of northern limit."""
        ...
        if len(lats_north) > 0 and len(lats_south) > 0:
            # Southern limit should be south of (lower latitude than) northern limit
            assert lats_south[0] < lats_north[0], (
                f"Southern limit ({lats_south[0]:.2f}) should be south of "
                f"northern limit ({lats_north[0]:.2f})"
            )

    def test_path_width_consistent_with_limits(self):
        """Test that the difference between northern and southern limits is reasonable."""
        from libephemeris import calc_eclipse_northern_limit
