"""
LEB vs Skyfield Comparison: Uranian Planets + Transpluto (Extended Tier).
...
class TestExtHypotheticalPosition:
    ...
    def test_position(
        ...
        assert max_lon_err < TOLS_EXT.HYPOTHETICAL_ARCSEC, (
            f'{body_name}: max lon error = {max_lon_err:.4f}" at JD {worst_jd:.1f}'
        )
        assert max_lat_err < TOLS_EXT.HYPOTHETICAL_ARCSEC, (
            f'{body_name}: max lat error = {max_lat_err:.4f}"'
        )


class TestExtHypotheticalSpeed:
