...
class TestLunarPrecision:
    ...
    def test_all_components(
        ...
        tol_lon = ECLIPTIC_TOLERANCES.get(body_id, {}).get("lon", TOLS.ECLIPTIC_ARCSEC)
        ...
        assert max_lon_err < tol_lon, (
            f'{body_name}: lon error {max_lon_err:.4f}" (tol={tol_lon})'
        )
        assert max_lat_err < tol_lat, (
            f'{body_name}: lat error {max_lat_err:.4f}" (tol={tol_lat})'
        )
        assert max_dist_err < tol_dist, (
            f"{body_name}: dist error {max_dist_err:.2e} AU (tol={tol_dist})"
        )
        assert max_speed_lon_err < tol_speed, (
