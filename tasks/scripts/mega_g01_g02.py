...
for y, m, d, h in KNOWN_DATES:
    try:
        ...
        check(
            jd_alias == jd_lib,
            f"swe_julday alias mismatch for ({y},{m},{d},{h})"
        )
        ...
        check(
            type(jd_lib) is float,
            f"julday returned {type(jd_lib).__name__} not float"
        )
...
print(f"  G01.01 julday:        {sum(n_g0101):>4} checks  (pass={n_g0101[0]}, fail={n_g0101[1]})")
...
print(f"  G01.02 revjul:        {sum(n_g0102):>4} checks  (pass={n_g0102[0]}, fail={n_g0102[1]})")
...
print(f"  G01.03 round-trip:    {sum(n_g0103):>4} checks  (pass={n_g0103[0]}, fail={n_g0103[1]})")
...
for jd in deltat_jds:
    try:
        ...
        if y_approx > 1905:
            check(
                dt_lib > 0,
                f"deltat({jd:.1f}) not positive: {dt_lib}"
            )
        else:
...
print(f"  G01.04 deltat:        {sum(n_g0104):>4} checks  (pass={n_g0104[0]}, fail={n_g0104[1]})")
...
print(f"  G01.05 deltat_ex:     {sum(n_g0105):>4} checks  (pass={n_g0105[0]}, fail={n_g0105[1]})")
...
for jd in sidtime_jds:
    try:
        ...
        check(
            0.0 <= st_lib < 24.0,
            f"sidtime({jd:.1f}) out of range: {st_lib}"
        )
    except Exception as e:
        for _ in range(2):
...
print(f"  G01.06 sidtime:       {sum(n_g0106):>4} checks  (pass={n_g0106[0]}, fail={n_g0106[1]})")
...
for jd in sidtime_jds[:25]:
    try:
        ...
        check(
            0.0 <= st_lib < 24.0,
            f"sidtime0({jd:.1f}) out of range: {st_lib}"
        )
    except Exception as e:
        for _ in range(2):
...
print(f"  G01.07 sidtime0:      {sum(n_g0107):>4} checks  (pass={n_g0107[0]}, fail={n_g0107[1]})")
...
for y, mo, d, h, mi, s in UTC_DATES:
    try:
        ...
        check(
            ...
            f"utc_to_jd returned non-float types"
        )
        ...
        check(
            ...
            f"swe_utc_to_jd alias mismatch"
        )
    except Exception as e:
        for _ in range(5):
...
print(f"  G01.08 utc_to_jd:     {sum(n_g0108):>4} checks  (pass={n_g0108[0]}, fail={n_g0108[1]})")
...
print(f"  G01.09 jd_to_utc:     {sum(n_g0109):>4} checks  (pass={n_g0109[0]}, fail={n_g0109[1]})")
...
print(f"  G01.10 time_equ:      {sum(n_g0110):>4} checks  (pass={n_g0110[0]}, fail={n_g0110[1]})")
...
print(f"  G01.11 day_of_week:   {sum(n_g0111):>4} checks  (pass={n_g0111[0]}, fail={n_g0111[1]})")
...
print(f"  G01.12 TAI:           {sum(n_g0112):>4} checks  (pass={n_g0112[0]}, fail={n_g0112[1]})")
...
for body in BODIES_MAIN:
    bn = BODY_NAMES[body]
    print(f"    {bn:>10s}: lon_max={lon_max_diff[body]:.3f}\"  lat_max={lat_max_diff[body]:.3f}\"  dist_max={dist_max_diff[body]:.2e} AU")

n_g0201 = section_counts["G02.01 geocentric"]
print(f"  G02.01 geocentric:    {sum(n_g0201):>4} checks  (pass={n_g0201[0]}, fail={n_g0201[1]})")
...
print(f"  G02.02 nodes/apsides: {sum(n_g0202):>4} checks  (pass={n_g0202[0]}, fail={n_g0202[1]})")
...
print(f"  G02.03 asteroids:     {sum(n_g0203):>4} checks  (pass={n_g0203[0]}, fail={n_g0203[1]})")
...
print(f"  G02.04 speed:         {sum(n_g0204):>4} checks  (pass={n_g0204[0]}, fail={n_g0204[1]})")
